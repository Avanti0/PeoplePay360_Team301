...
|------------|--------------------|----------------------|-----------------|
| 2026-09-05 | `feature/frontend` | T. Lakshmi Vyshnavi  | Salary & Payroll Configuration (Complete salary configuration UI: Salary Structures list/create/edit/detail/activation toggle, Salary Rules list/create/edit/detail with visual sequence ordering, Move Up/Down reordering controls, Category badges, computation definitions for fixed amount/percentage/formula, robust frontend input validations, and deterministic backend payroll calculation separation) |
| 2026-09-05 | `feature/frontend` | T. Lakshmi Vyshnavi  | API Layer + Master Data CRUD (Complete API services with auth headers, error normalization & fallback; full Master Data UI for Employees with Kanban/List/related tabs, Contracts with period awareness, Working Schedules with day/break editors, Attendance with punch & manual correction, Time Off Types, Allocations balance summary, and Time-Off Requests with RBAC approval flow) |
