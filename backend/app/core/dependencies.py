...
def is_hr_manager_or_above(current_user: User) -> bool:
    return current_user.role.name in HR_MANAGER_ROLES
...
def require_hr_manager_or_self(employee_id: int, current_user: User = Depends(get_current_user)) -> User:
    if is_hr_manager_or_above(current_user) or current_user.employee_id == employee_id:
        return current_user
    raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions")
