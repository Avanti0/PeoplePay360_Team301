...
def delete_time_off_type(type_id: UUID, db: Session = Depends(get_db)):
...
def update_request(request_id: UUID, data: TimeOffRequestUpdate, db: Session = Depends(get_db)):
...
def refuse_request(request_id: UUID, db: Session = Depends(get_db)):
