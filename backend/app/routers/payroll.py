...
def get_structure(structure_id: UUID, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
...
def list_rules(structure_id: Optional[UUID] = None, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
...
def get_rule(rule_id: UUID, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
...
def list_payslips(payrun_id: Optional[UUID] = None, employee_id: Optional[UUID] = None, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
    return svc.list_payslips(db, payrun_id, employee_id)

@payslips_router.get("/{payslip_id}", response_model=PayslipOut)
def get_payslip(payslip_id: UUID, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
    return svc.get_payslip(db, payslip_id)

@payslips_router.get("/{payslip_id}/pdf")
def download_pdf(payslip_id: UUID, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
