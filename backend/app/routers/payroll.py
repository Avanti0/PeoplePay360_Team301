...
def list_structures(db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
...
def get_structure(structure_id: int, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
    return svc.get_structure(db, structure_id)

@structures_router.put("/{structure_id}", response_model=SalaryStructureOut)
def update_structure(structure_id: int, data: SalaryStructureUpdate, db: Session = Depends(get_db), _=Depends(require_hr_payroll_manager)):
...
def list_rules(structure_id: int = None, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
...
def get_rule(rule_id: int, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
    return svc.get_rule(db, rule_id)

@rules_router.put("/{rule_id}", response_model=SalaryRuleOut)
def update_rule(rule_id: int, data: SalaryRuleUpdate, db: Session = Depends(get_db), _=Depends(require_hr_payroll_manager)):
...
def create_payrun(data: PayrunCreate, db: Session = Depends(get_db), current_user=Depends(require_hr_payroll_user)):
    return svc.create_payrun(db, data, created_by=current_user.id)

@payruns_router.get("/{payrun_id}", response_model=PayrunOut)
def get_payrun(payrun_id: int, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
    return svc.get_payrun(db, payrun_id)

@payruns_router.post("/{payrun_id}/compute", response_model=PayrunOut)
def compute(payrun_id: int, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
    return svc.compute_payrun(db, payrun_id)

@payruns_router.post("/{payrun_id}/validate", response_model=PayrunOut)
def validate(payrun_id: int, db: Session = Depends(get_db), _=Depends(require_hr_payroll_manager)):
...
def list_payslips(payrun_id: int = None, employee_id: int = None, db: Session = Depends(get_db), _=Depends(require_hr_payroll_user)):
